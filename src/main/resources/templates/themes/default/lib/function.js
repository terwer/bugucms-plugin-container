...
//c_id:复选框隐藏与服务器交互ID值
//c_name:复选框页面显示name组名称
function radio_show(c_id,g_name) {
    var xx = $("#" + c_id).val().split(",");
    $.each(xx, function(i, val) {
        $("#"+g_name+" input[type='radio']").each(function() {
            if ($(this).val() == val) {
                $(this).attr("checked", 'true');
            }
        });
...
function radio_add(c_id,g_name) {
	
    $("#"+g_name+" input[type='radio']").click(function() {
        var chk_value = [];
        $("#"+g_name+" input[type='radio']:checked").each(function() {
            chk_value.push($(this).val());
        });
        $("#" + c_id).val(chk_value);
    });
