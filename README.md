...
```
mvn install:install-file -Dfile=target/ROOT.war -DgroupId=com.terwergreen -DartifactId=bugucms-plugin-container -Dversion=2.1.0 -Dpackaging=war
```

## 当做parent引用的pom.xml

```
mvn install:install-file -Dfile=pom-parent.xml -DgroupId=com.terwergreen -DartifactId=bugucms-plugin-container -Dversion=2.1.0 -Dpackaging=pom
```
...
# 启动
## 注意
（1）ojdbc6.jar需要手动安装，在项目的lib目录下，需要用命令```mvn install:install-file -DgroupId=com.oracle -DartifactId=ojdbc6 -Dversion=11.2.1.0 -Dpackaging=jar -Dfile=ojdbc6.jar```安装        
